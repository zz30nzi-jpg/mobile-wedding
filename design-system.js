(function () {
  ...
  function normalize(data = {}, library = null) {
    ...
    const defaultPrompts = {
      ...
      transport: "예식장 기준 자가용, 버스, 지하철 3가지를 기본으로 안내한다. 자가용은 식장 공식 안내를 우선해 주차장 위치와 외부 주차장 여부를 확인한다. 버스/지하철은 먼저 식장 도보 20분 이내에 버스정류장과 지하철역이 각각 있는지 판정한다. 둘 다 있으면 각 이름과 도보 시간을 적고, 하나만 있으면 가까운 접근수단은 이름과 도보 시간을, 다른 접근수단은 그 지점에서 가까운 접근수단 또는 식장까지 이어지는 버스 번호/타는 정류장과 택시 시간을 함께 적는다. 불확실한 정보는 확인 필요라고 표시한다.",
