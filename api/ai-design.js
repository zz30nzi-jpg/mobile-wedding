  ...
  properties: {
    items: {
      ...
      items: {
        type: "object",
        properties: {
          title: { type: "string", enum: ["자가용", "버스", "지하철", "기차", "셔틀버스"] },
          text: { type: "string" },
        },
        required: ["title", "text"],
...
${prompts.transport || ""}
요구사항:
- title에는 다음 값만 사용하세요: "자가용", "버스", "지하철", "기차". "자가용"과 "버스"는 반드시 포함하고, 철도 안내는 아래 규칙에 따라 "지하철"과 "기차" 중 실제로 존재하는 것만 포함하세요. 출력 순서는 "자가용" → "버스" → ("기차" 또는 "지하철") 순을 권장합니다.
- 각 항목의 text는 줄바꿈으로 구분된 2줄을 기본으로 작성하고, 안내가 더 필요하면 최대 4줄까지 줄을 추가하세요. 이모지나 아이콘은 넣지 마세요(화면에 자동으로 표시됩니다).
- 각 줄에는 정보 1개만 적으세요. 정류장/역 이름, 도보 시간, 환승 버스, 택시 시간은 반드시 서로 다른 줄로 분리하세요.
- 한 줄에 "에서", "타고", "택시", "도보", "식장" 정보를 모두 합친 긴 문장을 만들지 마세요.
- 항목별 text는 문단이 아니라 목록 데이터처럼 보여야 합니다. 예: "OO정류장", "정류장에서 도보 약 4분"
- "참고", "공식 홈페이지", "블로그", URL, 전화번호, 출처명은 text 줄에 쓰지 마세요. 출처 설명은 하객 안내가 아니므로 제외하세요.
- 확실하지 않은 항목은 그 사실을 길게 설명하지 말고 "확인 필요"라고만 적으세요. 관리 화면에서 숨김 처리될 수 있습니다.
...
- 철도 안내는 "지하철"(도시철도/전철/메트로/호선)과 "기차"(KTX·SRT·일반철도 등 기차역)를 명확히 구분하세요. 같은 역을 두 카테고리에 중복으로 넣지 마세요.
- 식장 인근(또는 하객이 현실적으로 이용할) 지하철역이 있으면 "지하철" 항목을, 기차역(KTX/SRT/일반철도)이 있으면 "기차" 항목을 작성하세요. 둘 다 있으면 둘 다, 하나만 있으면 그 하나만, 둘 다 없으면 철도 항목은 생략하세요. 지하철역이 없는 지역에서 기차역을 "지하철"이라고 부르지 마세요(반대도 마찬가지).
- 버스/지하철/기차 각 항목은: 1줄 역·정류장 이름(지하철은 호선/역명), 2줄 도보 N분 또는 식장까지 택시 약 N분, 필요하면 3줄 연계 버스 번호/타는 정류장 순으로 작성하세요.
- 도보 가능 여부, 소요시간, 버스 번호는 지도/공식 접근 안내/검색 결과에 근거하세요. 근거가 없으면 도보 가능하다고 단정하지 말고 "확인 필요"라고 적으세요.
...
- 식장 기준 인근 버스정류장명과 식장까지 도보 소요시간
- 식장 인근 지하철역(도시철도/전철/메트로) 존재 여부와 호선/역명, 식장까지 도보 또는 택시 소요시간 (지하철이 없는 지역이면 "지하철역 없음"이라고 명시)
- 식장 인근 기차역(KTX/SRT/일반철도) 존재 여부와 역명, 식장까지 도보 또는 택시 소요시간 (기차역이 없으면 "기차역 없음"이라고 명시)
- 지하철역과 기차역은 반드시 구분해서 적으세요. 같은 도시라도 지하철이 없고 기차역(KTX 등)만 있는 경우가 많습니다.
- 역/정류장에서 식장까지 연계되는 버스 번호와 타는 정류장, 택시 소요시간`;
  }
  return `${common}
